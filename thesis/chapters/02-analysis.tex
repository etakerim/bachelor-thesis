...
\Function{Find\_Peaks\_Neighbours}{$y$, $k$, $\varepsilon$, $h_{rel}$, $h$}
	\State $peaks \gets \left[\; \right]$ 	\Comment{Zoznam indexov nájdených špičiek v signále $y$}
	\For{$i \gets 0$ \textbf{to} $length(y)$}
		\If {$y[i] \geq h$}  						\Comment{Preskoč príliš nízke amplitúdy}
			\State $possible\_peak \gets true$
			\State $a \gets max(i - k, 0)$
			\State $b \gets min(i + k, length(y))$
			\State $valley \gets y[a]$

			\For{$j \gets a$ \textbf{to} $b$}		\Comment{Porovnanie špičky s bodmi v susedstve}
				\If {$i \neq j$}
					\If {$y[j] - y[i] > \epsilon$}	\Comment{Tolerancia pre vyšší bod v susedstve}
						\State $possible\_peak \gets false$
					\EndIf
					\If {$y[j] < valley$}			\Comment{Údolie v susedstve na relatívnu výšku}
						\State $valley \gets y[j]$
					\EndIf
				\EndIf
			\EndFor
			\If {$possible\_peak = true$ \textbf{and} $y[i] - valley \geq h_{rel}$}
				\State $peaks \gets peaks + [i]$    \Comment{Kandidát je prehlásený za špičku}
			\EndIf
		\EndIf
	\EndFor
	\State \Return $peaks$
...
\begin{algorithmic}[1]
\Function{Find\_Peaks\_Zero\_Crossing}{$y$, $k$, $slope$}
	\State $peaks \gets \left[\; \right]$
	\For{$i \gets k$ \textbf{to} $length(y) - k$}
		\If {($y[i + k] - y[i]) < 0$ \textbf{and} $(y[i] - y[i - k]) > 0$
		     \State \hskip1.5em $|y[i + k] - y[i]| \geq slope$ \textbf{and} $|y[i] - y[i - k]| \geq slope$)}
	...
	\For{$i \gets 1$ \textbf{to} $length(y)$}
        ...
        \If {($possible\_change = true$
            ...
            \ElsIf {($prev\_uphill = true$
				...
				\State $peaks \gets peaks + [i\_change]$
            \EndIf
        \EndIf
	\EndFor
	\State \Return $peaks$
