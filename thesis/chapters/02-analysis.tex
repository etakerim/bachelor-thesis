...
Časový rad $\left(x_i\right)_{i = 0}^{n}$ s dĺžkou $n$ môže byť pre účely výpočtu sumárnych štatistík rozdelený okennou funkciou 
...
separovaný do blokov posuvným oknom. Rozobrali sme algoritmus opierajúci sa o porovnávanie susedov na obe strany, 
...
Periodické deje v signále sú extrahované zo sekvencie vzoriek v časovej doméne transformovaným do domény frekvenčnej.
...
znamenajúci, že pri raste rozlíšenia v čase strácame rozlíšenie vo frekvenciách a naopak. O požadovanom množstve pozorovaní 
pre konkrétnu granularitu spektra taktiež hovorí vzťah (\ref{equ:freq-resolution}). Rozpätie frekvencií spadajúce do
diskrétneho frekvenčného vedierka $k$ sú počínajúc $k\Delta f$ po $(k+1)\Delta f$.
 
\subsection{Diskrétna fourierová a kosínusová transformácia}
Diskrétna fourierová transformácia (DFT) je aparát na učenie harmonického zloženia signálu. Zobrazuje vektor komplexných
čísel $y$ dĺžky $N$ do vektora $N$ frekvenčných komponentov. Na vypočítanie $k$-teho frekvenčného vedierka sú 
prvky sekvencie pozorovaní prenásobené zodpovedajúcim exponenciálnym členom tzv. \emph{twiddle factor}. Inverzná transformácia
sa líši iba opačným znamienkom exponenta a získané hodnoty sa zvyknú normovať podelením $N$.
\begin{ceqn}\begin{align}
\tag{DFT} Y[k] &= \sum_{n=0}^{N - 1}{x[n] \cdot W_N^{nk}};\quad k = 0, \dots, N - 1 \\
W_{N}^{nk} &= \exp{\left(-i2\pi nk \, /\, N\right)}
\end{align}\end{ceqn}

Alternatívne sa pre praktické aplikácie vyjadruje exponenciálny faktor v goniometrickom tvare:
\begin{ceqn}\begin{align}
...
(\ref{equ:magnitude-spectrum}). Pre reálne čísla je výstup z DFT zrkadlovo symetrický, čo zapríčiňuje nadbytočnosť výsledkov
nad pozíciou  $N/2$. Rovnako naznačuje reformulácia vety o vzorkovaní, že za vzorkovacej frekvencie $f_s$ sú zapríčinením aliasingu v signále prítomné  frekvencie do maximálne polovice $f_s$. Energia nesená frekvenciou je druhou mocninou magnitúdy, ale častejšie sa 
...
%TODO
Kosínusová transfomácia (DCT) (DCT-II, DCT-III (inverzná k II), DCT-IV (sama sebe inverzom)),

Koeficienty pre kosínusové transformácie\cite{dct}
\begin{ceqn}\begin{align}
...
\cite{fft-blackbox} \cite{fast-dct} \cite{dit-dif-fft} \cite{computer-vision-fft}


The computational complexity of DFT is substantially reduced by using the following
kntrigonometric symmetry and periodicity of the twiddle factor 

N koplexných sčítaní, N/2 komplexných násobení

Decimácia v čase vs. Decimácia vo frekvencii. 
Motýlikový diagram Cooley-Tukey vs. Gentleman-Sande
twiddle factor a ich uloženie v pamäti + predvypočítavanie
Opis DIT radix-2 FFT algoritmus
...
\begin{subfigure}[b]{0.48\textwidth}
    ...
    \label{fig:dif-fft}
\end{subfigure}
\caption{FFT s radixom 2 na štyroch bodoch \cite{dit-dif-fft}}
\end{figure}


\subsection{Okenné funkcie}
Oknové funkcie ováhujú vstupný vektor.Krátkodobá FT
DFT implicitne predpokladá periodickosť, že časový rad dĺžky N sa opakuje cyklicky do nekonečna. Ak frekvencia
sínusového vstupu nie je presným násobkom frekvenčného rozlíšenia, čiže nespadá presne do centra frekvenčného 
vedierka (bin), tento predpoklad nie je naplnený a DFT sa bude chovať akoby v prítomnosti nespojitosti
medzi poslednou a prvou vzorkou, z dôvodu cyklického opakovania. Diskontinuita sa rozšíri naprieč spektrom prejavom 
Gibbsovho javu. 
 
Oknové funkcie vyžadujú kompromis medzi šírkou výsledných špičiek v frekvenčnej doméne, presnosti v amplitúde
a miera poklesu úniku spektra (spectral leakage) do ostatných frekvenčných vedierok. \cite{spectral-density-estimation}
...
Prehľad okien - symetrické okolo stredu \cite{understanding-dsp}
\begin{ceqn}\begin{align}
\tag{Obdĺžníkové} w(n) &= 1,\, n = 0, 1, ..., N - 1 \\
\tag{Trojuholníkové} 
	w(n) &= 
	\begin{cases} 
	...
	\end{cases} \\
\tag{Welch} w(n) &= 1 - ((2n / N) - 1)^2 \\
\tag{Hann} w(n) &= \cos^2((2\pi n / N - \pi) / N)  \\
\tag{Hamming} w(n) &= 0.54 - 0.46\cos(2\pi n / N)
\end{align}\end{ceqn}

výhodné percentá prekryvu FT, čím užšie okno tým vyžadujú väčší prekryv
...
Welchova metóda na zníženie šumu, spriemerovanie spektier z po sebe idúcich úsekov na zníženie rozpylu pri spektre stochastického
javu.
...
\label{fir-filter}
Roziel medzi FIR a IIR, Dolná pripusť, pásmová priepusť, horná pripusť, pre ideálny filter 
 Konvolúcia a konvolučné jadro, konvolučná veta, účel: identifikácia prítomnosti známej frekvencie v signále akcelerácie
\begin{ceqn}\begin{align}
y[k] = \sum_{k=0}^{N - 1}{x[k] \cdot h(n - k)} = x[n] * h[n]
\end{align}\end{ceqn} 
 
Prenosová funkcia - koeficienty filtra
\cite{signal-processing} s.125 + popísať, že je možné aplikovať FFT pri výpočte konvolúcie Konvolučná veta
Frekvenčná odozva filtra je daná fourierovu transformáciu sinc, keď chceme fitrovať nejakú frekv. musíme
aplikovať sinc kernel (maska)
...
Nízko-energetické zariadenia komunikujúce cez odľahčené sieťové protokoly so snahou spracovania v reálnom čase a ponechaním najdôležitejších informácii dolovaním z veľkého množstva zdrojových dát. Cloud / Hmlové počítanie. 
Edge computing okrajový a sink (zoskupujúci) uzol \cite{edge-computing-survey} \cite{edge-vision} \cite{edge-overview}
...
distributed computing divides a single task between multiple computers to achieve a common goal

address the same problem: proximity. Moving the computer workload closer to the consumer reduces latency, bandwidth and overhead for the centralized data center, komunikujú a koorinujú svoje aktivity výmenou / zasielaním (predávaním) správ medzi komponentami,
pamäť je oddelená

Improved response time, Offline availability, Offline availability

Like the edge, fog computing moves the workload closer to the network edge, reducing data travel, latency and bandwidth. Whereas edge computing moves the process to devices, though, fog computing happens across one or more nodes in a network.
