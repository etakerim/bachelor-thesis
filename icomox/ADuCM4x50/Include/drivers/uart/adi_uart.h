 ...
 */
#define ADI_UART_UNIDIR_MEMORY_SIZE  (48u + (72u + ADI_SEM_SIZE))

/*! Amount of memory(bytes) required by the UART device driver for operating bidirectionally(Both RX and TX).
 *  This memory is completely owned by the driver until the end of the operation.
 */
#define ADI_UART_BIDIR_MEMORY_SIZE  (48u + (72u + ADI_SEM_SIZE)*2u)
...
/*!
 *****************************************************************************
 * \struct ADI_UART_RX_BUFFER_INFO
 *  Structure for managing Rx buffers.
 *****************************************************************************/

typedef struct ADI_UART_RX_BUFFER_INFO
{
    void                      *pStartAddress;             /*!< Address of buffer passed down to the UART driver.    */
    uint32_t                   nBufferSize;               /*!< Number of bytes a buffer can store.                  */
    uint32_t                   nBytesNumber;              /*!< Number of bytes received.                            */
} ADI_UART_RX_BUFFER_INFO;                                /*!< Structure for managing Rx buffers.                   */
