...
#define ADI_CRYPTO_MEMORY_SIZE          (100u + ADI_SEM_SIZE) /*!< Required user memory size for ADuCM4x50 processor family. */
#else
#define ADI_CRYPTO_MEMORY_SIZE          (92u + ADI_SEM_SIZE)  /*!< Required user memory size for ADuCM4x50 processor family. */
#endif
#elif defined (__ADUCM302x__)
#define ADI_CRYPTO_MEMORY_SIZE          (88u + ADI_SEM_SIZE)  /*!< Required user memory size for ADuCM302x processor family. */
#else
...
typedef enum
{
    ADI_CRYPTO_SHA_LITTLE_ENDIAN = (0u << BITP_CRYPT_CFG_SHA_BYTESWAP),    /*!< Do not apply SHA/HMAC data write byte swaps. */
    ADI_CRYPTO_SHA_BIG_ENDIAN    = (1u << BITP_CRYPT_CFG_SHA_BYTESWAP),    /*!< Apply SHA/HMAC data write byte swaps. */
...
#if defined (__ADUCM4x50__)
#if ((1 == ADI_CRYPTO_ENABLE_PKSTOR_SUPPORT) || (1 == ADI_CRYPTO_ENABLE_HMAC_SUPPORT))
...
#endif /* ADI_CRYPTO_ENABLE_PKSTOR_SUPPORT or ADI_CRYPTO_ENABLE_HMAC_SUPPORT */
#endif /* __ADUCM4x50__ */
...
typedef struct
{
    ...
    uint8_t                  *pAesKey;            /*!< Pointer to the KEY data: pre-formatted as a byte array, according to eAesKeyLen. */
    ADI_CRYPTO_AES_KEY_LEN    eAesKeyLen;         /*!< The length of the AES KEY */

    uint8_t                  *pHmacKey;           /*!< Pointer to the 512-bit/32-bytes HMAC KEY data: pre-formatted as a byte array. */
    ...
    /* NOTE: Enabling PKSTOR boolean results in explicit key loads being replaced with PKSTOR keys prior to all Crypto operations */
    ...
    uint8_t                   pkIndex;           /*!< Flash index within PKSTOR for storing/retrieving keys. */
#endif
