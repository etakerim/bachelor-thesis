...
ADI_PWR_RESULT adi_pwr_EnableClockSource (const ADI_CLOCK_SOURCE_ID eClockSource, const bool bEnable)
{
    ...
    {
        *pReg |= val;
        if( eClockSource == ADI_CLOCK_SOURCE_SPLL )
        {
          /* Wait for the PLL to lock */
          uint32_t read_status;  
          do {
           read_status = ((*pREG_CLKG0_CLK_STAT0 & BITM_CLKG_CLK_STAT0_SPLL)>>BITP_CLKG_CLK_STAT0_SPLL);
 ...
 *
 * @note        The clock signal is outputted on Port 2 Pin 11
 * @note        P2_11 must be muxed as SYS_CLKOUT. This API does not
 * @note        alter the muxing. The application must ensure that P2_11
 * @note        is muxed correctly.
 *
 * @return      Status
                                         ...
                                         )
{
    ...
    ADI_ENTER_CRITICAL_REGION();
		...
		/* NOTE: the watchdog timer(WDT)is reset by the core hardware with every exit from low-power mode.
 		   Therefore,even though we may have disabled it during startup, it will reset
        ...
        */
        ...
        /* Save DMA STATUS register  */
        savedDMA_STAT = pADI_DMA0->STAT;
        ...
        /* re-enable DMA if it was enable before  */
        if (savedDMA_STAT  & BITM_DMA_STAT_MEN) {
...
ADI_PWR_RESULT adi_pwr_ExitLowPowerMode(uint32_t volatile * pnInterruptOccurred)
{
    ...
    if (NULL == pnInterruptOccurred) {
        ...
        ADI_ENTER_CRITICAL_REGION();
            ...
            /* Ensure hibernate/shutdown modes are cleared and flexi mode set */
            pADI_PMG0->PWRMOD &= (uint32_t) (~BITM_PMG_PWRMOD_MODE);

            /* Clear System Control Register bits SLEEPDEEP and SLEEPONEXIT */
