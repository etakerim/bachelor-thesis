                             ...
                             )
{
    ...
    */
    ...
    /* Extract the FIFO and FIFO trigger settings.  These are required
     * in the Rx interrupt handler to extract the right amount of data
     * on each Rx interrupt */
 ...
 * @param [in]  nBufSize   Size of the buffer to be transmitted(in bytes). In DMA Scatter-Gather mode, this is the number of descriptors.
                                       ...
                                       )
{
    ...
    */
    ...
    if(bDMA == true)
    {
        if(nBufSize > DMA_TRANSFER_LIMIT)
        {
#if !defined(ADI_UART_DMA_EXTENSION_ENABLE) || (0 == ADI_UART_DMA_EXTENSION_ENABLE)
            return(ADI_UART_INVALID_DATA_SIZE);
#endif
        }
#if defined(ADI_UART_DMA_EXTENSION_ENABLE) && (ADI_UART_DMA_EXTENSION_ENABLE == 1)
        if(hDevice->pChannelTx->pFreeBuffer->nDmaMode == DMA_ENUM_CTL_CYCLE_CTL_PSG_PRI)
        {
            ADI_DCC_TypeDef* pBuffer1 = pBuffer;

            /* Check if the cycle control of the descriptors is valid */
            for(uint32_t cnt = 0u; cnt < (nBufSize-1u); cnt++)
            {
                if((pBuffer1[cnt].DMACDC & DMA_BITM_CTL_CYCLE_CTL) != DMA_ENUM_CTL_CYCLE_CTL_PSG_ALT)
                {
                    return ADI_UART_INVALID_PARAMETER;
                }
            }

            /* Validate the cycle control of the last descriptor */
            if((pBuffer1[nBufSize-1u].DMACDC & DMA_BITM_CTL_CYCLE_CTL) != DMA_ENUM_CTL_CYCLE_CTL_BASIC)
            {
                return ADI_UART_INVALID_PARAMETER;
            }
        }
    ...
    /* Initialize the value of nRemDoneTransfers with the size of the buffer  */
    hDevice->pChannelTx->pFreeBuffer->nRemDoneTransfers = nBufSize;
    ...
    */
    ...
    /* Initialize to select the Primary Data Control Structure */
    hDevice->pChannelTx->bIsPrimNext                =   true;

    /* Set the number of free descriptors */
    hDevice->pChannelTx->nFreeDescriptor            =   2U;

    if(bDMA == true)
    {
        /* Initialize the DMA channel uses the primary control data structure */
        pADI_DMA0->ALT_CLR = 1u << hDevice->pUartInfo->dmaTxChannelNum;
#if !defined(ADI_UART_DMA_EXTENSION_ENABLE) || (0 == ADI_UART_DMA_EXTENSION_ENABLE)
        /* Set the DMA Mode to Basic */
        hDevice->pChannelTx->pFreeBuffer->nDmaMode = DMA_ENUM_CTL_CYCLE_CTL_BASIC;
#else

        /*If not previously set, configure the default DMA mode to Basic */
        if(hDevice->pChannelTx->pFreeBuffer->nDmaMode == DMA_ENUM_CTL_CYCLE_CTL_INVALID)
        {
            hDevice->pChannelTx->pFreeBuffer->nDmaMode = DMA_ENUM_CTL_CYCLE_CTL_BASIC;
        }
        ...
        if (nBufSize > DMA_TRANSFER_LIMIT)
        {
            /* Set the mode to Ping-Pong*/
            hDevice->pChannelTx->pFreeBuffer->nDmaMode = DMA_ENUM_CTL_CYCLE_CTL_PING_PONG;
        }

        if (hDevice->pChannelTx->pFreeBuffer->nDmaMode == DMA_ENUM_CTL_CYCLE_CTL_PSG_PRI)
        {
            /* Set the size of the buffer. For scatter-gather, this is the transaction size.
               The size is multiplied by 4 since descriptors are being transferred when
               using this mode and each descriptor is a channel control data structure that contains
               four four-bytes members.
            */
            hDevice->pChannelTx->pFreeBuffer->nCount            =   4u * nBufSize;

            /* Initialize the value of nRemDoneTransfers with the size of the buffer  */
            hDevice->pChannelTx->pFreeBuffer->nRemDoneTransfers =   4u * nBufSize;
        }
 #endif
    }
                               ...
                               )
{
    ...
    if (pBuffer->bDMA)
    {
...
#else
        /* Pointer to the Channel Control Data Structure. */
        ADI_DCC_TypeDef* pCCD;

        /* Pointer to the Uart Data Channel. */
        ADI_UART_DATA_CHANNEL* pChannel = hDevice->pChannelTx;

        /* Number of bytes to be processed. */
        uint16_t nTransactions = 0;

        /* Variable that holds the control data configuration. */
        uint32_t nCfg = 0;
        ...
        while(pBuffer->nCount > 0 && pChannel->nFreeDescriptor > 0)
        {
            if(pBuffer->nCount < 2*DMA_TRANSFER_LIMIT)
            {
                if(pBuffer->nCount > DMA_TRANSFER_LIMIT)
                {
                    nTransactions = pBuffer->nCount/2;
                }
                else
                {
                    nTransactions = pBuffer->nCount;
                }
            }
            else
            {
                nTransactions = DMA_TRANSFER_LIMIT;
            }

            if (pChannel->bIsPrimNext)
            {
                /* Set the CCD pointer to point to the Primary Data Structure */
                pCCD                  = &pPrimaryCCD[hDevice->pUartInfo->dmaTxChannelNum];
                pChannel->bIsPrimNext = false;
            }
            else
            {
                /* Set the CCD pointer to point to the Alternate Data Structure */
                pCCD                  = &pAlternateCCD[hDevice->pUartInfo->dmaTxChannelNum];
                pChannel->bIsPrimNext = true;
            }

            /* Decrement the number of available descriptors */
            pChannel->nFreeDescriptor--;

            if (pBuffer->nDmaMode == DMA_ENUM_CTL_CYCLE_CTL_PSG_PRI)
            {
                nCfg = ((uint32_t)ADI_DMA_INCR_4_BYTE << DMA_BITP_CTL_DST_INC) |
                       ((uint32_t)ADI_DMA_INCR_4_BYTE << DMA_BITP_CTL_SRC_INC) |
                       (ADI_DMA_WIDTH_4_BYTE << DMA_BITP_CTL_SRC_SIZE)         |
                       (ADI_DMA_RPOWER_4 << DMA_BITP_CTL_R_POWER)              |
                       ((nTransactions-1) << DMA_BITP_CTL_N_MINUS_1)           |
                       (pBuffer->nDmaMode<< DMA_BITP_CTL_CYCLE_CTL);

                dstAddr = (uint32_t)&pAlternateCCD[hDevice->pUartInfo->dmaTxChannelNum].RESERVED;
            }

            else
            {
                nCfg = ((uint32_t)ADI_DMA_INCR_NONE << DMA_BITP_CTL_DST_INC)   |
                       ((uint32_t)ADI_DMA_INCR_1_BYTE << DMA_BITP_CTL_SRC_INC) |
                       (ADI_DMA_WIDTH_1_BYTE << DMA_BITP_CTL_SRC_SIZE)         |
                       (ADI_DMA_RPOWER_1 << DMA_BITP_CTL_R_POWER)              |
                       ((nTransactions-1) << DMA_BITP_CTL_N_MINUS_1)           |
                       (pBuffer->nDmaMode<< DMA_BITP_CTL_CYCLE_CTL);

                dstAddr = (uint32_t)&hDevice->pUARTRegs->TX;

            }

            /* Retrieve the source increment */
            srcInc = 1u << ((nCfg & DMA_BITM_CTL_SRC_INC) >> DMA_BITP_CTL_SRC_INC);

            /* Fill in the DMA descriptors */
            pCCD->DMASRCEND = (uint32_t)pBuffer->pStartAddress + ((nTransactions-1) + pBuffer->nIndex) * srcInc;

            pCCD->DMADSTEND = dstAddr;

            pCCD->DMACDC    = nCfg;

            /* Update the nCount and nIndex */
            pBuffer->nCount -= nTransactions;
            pBuffer->nIndex += nTransactions;
        }
#endif

        /* Protecting IEN register writes by disabling interrupts ensures the interrupt handler
        also doesn't modify the same register concurrently */
 ...
 * @param [in]  nBufSize   Size of the buffer(in bytes). In DMA Scatter-Gather mode, this is the number of descriptors.
                                       ...
                                       )
{
    ...
    */
    ...
    /* Set the DMA mode to Ping Pong if the DMA transfer size exceeds the maximum transfer
       limit. */
    if(bDMA == true)
    {
        if(nBufSize > DMA_TRANSFER_LIMIT)
...
#if defined(ADI_UART_DMA_EXTENSION_ENABLE) && (ADI_UART_DMA_EXTENSION_ENABLE == 1)
        if(hDevice->pChannelRx->pFreeBuffer->nDmaMode == DMA_ENUM_CTL_CYCLE_CTL_PSG_PRI)
        {
            ADI_DCC_TypeDef* pBuffer1 = pBuffer;
    ...
    /* Initialize the value of nRemDoneTransfers with the size of the buffer  */
    hDevice->pChannelRx->pFreeBuffer->nRemDoneTransfers = nBufSize;
    ...
    */
    ...
    /* Initialize to select the Primary Data Control Structure */
    hDevice->pChannelRx->bIsPrimNext                =   true;

    /* Set the number of free descriptors */
    hDevice->pChannelRx->nFreeDescriptor            =   2;
    ...
    if(bDMA == true)
    {
...
#if !defined(ADI_UART_DMA_EXTENSION_ENABLE) || (0 == ADI_UART_DMA_EXTENSION_ENABLE)
        /* Set the DMA Mode to Basic */
        hDevice->pChannelRx->pFreeBuffer->nDmaMode = DMA_ENUM_CTL_CYCLE_CTL_BASIC;
#else
        /*If not previously set, configure the default DMA mode to Basic */
        if(hDevice->pChannelRx->pFreeBuffer->nDmaMode == DMA_ENUM_CTL_CYCLE_CTL_INVALID)
        {
            hDevice->pChannelRx->pFreeBuffer->nDmaMode = DMA_ENUM_CTL_CYCLE_CTL_BASIC;
        }
        ...
        {
            /* Set the mode to Ping-Pong*/
            hDevice->pChannelRx->pFreeBuffer->nDmaMode = DMA_ENUM_CTL_CYCLE_CTL_PING_PONG;
        }
        ...
        if(hDevice->pChannelRx->pFreeBuffer->nDmaMode == DMA_ENUM_CTL_CYCLE_CTL_PSG_PRI)
        {
            /* Set the size of the buffer. For scatter-gather, this is the transaction size.
               ...
               four four-bytes members.
            */
            hDevice->pChannelRx->pFreeBuffer->nCount            =   4u * nBufSize;

            /* Initialize the value of nRemDoneTransfers with the size of the buffer  */
            hDevice->pChannelRx->pFreeBuffer->nRemDoneTransfers =   4u * nBufSize;
        }
#endif
    }
    /* Now that this "pFreeBuffer" is no longer free for use, update the
                               ...
                               )
{
    ...
    if (pBuffer->bDMA)
    {
        ...
        /* Pointer to the Uart Rx   Data Channel. */
        ADI_UART_DATA_CHANNEL* pChannel = hDevice->pChannelRx;
        ...
        while (pBuffer->nCount > 0 && pChannel->nFreeDescriptor > 0)
        {
            ...
            {
                /* Set the CCD pointer to point to the Primary Data Structure */
                pCCD= &pPrimaryCCD[hDevice->pUartInfo->dmaRxChannelNum];
                pChannel->bIsPrimNext = false;
            }
            else
            {
                /* Set the CCD pointer to point to the Alternate Data Structure */
                pCCD= &pAlternateCCD[hDevice->pUartInfo->dmaRxChannelNum];
                pChannel->bIsPrimNext = true;
            }
            ...
            if (pBuffer->nDmaMode == DMA_ENUM_CTL_CYCLE_CTL_PSG_PRI)
            {
                ...
                /* Retrieve the src increment */
                srcInc = 1u << ((nCfg & DMA_BITM_CTL_SRC_INC) >> DMA_BITP_CTL_SRC_INC);

                srcAddr = (uint32_t)pBuffer->pStartAddress + ((nTransactions-1) + pBuffer->nIndex) * srcInc;
                dstAddr = (uint32_t)&pAlternateCCD[hDevice->pUartInfo->dmaRxChannelNum].RESERVED;
            }

            else
            {

                nCfg = ((uint32_t)ADI_DMA_INCR_1_BYTE << DMA_BITP_CTL_DST_INC) |
                       ((uint32_t)ADI_DMA_INCR_NONE << DMA_BITP_CTL_SRC_INC)   |
                       (ADI_DMA_WIDTH_1_BYTE << DMA_BITP_CTL_SRC_SIZE)         |
                       (ADI_DMA_RPOWER_1 << DMA_BITP_CTL_R_POWER)              |
                       ((nTransactions-1) << DMA_BITP_CTL_N_MINUS_1)           |
                ...
                /* Retrieve the dst increment */
                dstInc = 1u << ((nCfg & DMA_BITM_CTL_DST_INC) >> DMA_BITP_CTL_DST_INC);

                srcAddr = (uint32_t)&hDevice->pUARTRegs->RX;
                dstAddr = (uint32_t)pBuffer->pStartAddress + ((nTransactions-1) + pBuffer->nIndex) * dstInc;

            }

                /* Fill in the DMA RAM descriptors. */
                pCCD->DMASRCEND = srcAddr;
...
/*!
 * @brief       Read the UART receive buffer register regardless of an error.
 *              This mode of reading a byte is done without taking into consideration the buffer
 *              handling mechanisms of UART IO/DMA/FIFO modes.
 *
 * @param [in]  hDevice    Device handle obtained from adi_uart_Open().
 * @param [in]  pRX        Pointer to a location where Rx is to be written.
 *
 * @return      Status
 *    - #ADI_UART_SUCCESS              Successfully returned buffer to the API.
 ...
 *
 *
 * @note: If this function is called before the receive buffer full interrupt is received, the previously
 *        received data is read again. For safer blocking read operations use adi_uart_Read()
*/
ADI_UART_RESULT adi_uart_GetRX(
                              ADI_UART_HANDLE const hDevice,
                              uint8_t        *const pRX
                             )
{
...
/*!
 * @brief        Set the DMA Mode of the UART Tx Channel
 *
 * @details      Sets the DMA Mode when ADI_UART_DMA_EXTENSION_ENABLE is enabled.
 *               Supported DMA Modes: Basic, Peripheral Scatter-Gather and Ping-Pong.
 *               When the ADI_UART_DMA_EXTENSION_ENABLE macro is disabled, which means
 *               that DMA Basic Mode is only supported, this function is unaccessible
 *               and should not be used since the default mode is already set to Basic.
 *               If used without setting the ADI_UART_DMA_EXTENSION_ENABLE to 1, this will
 *               result to build error.
 *
 *
 *
 * @param [in]  hDevice      Device handle obtained from adi_uart_Open().
 * @param [in]  eMode        DMA mode/cycle control
 ...
 */
#if defined(ADI_UART_DMA_EXTENSION_ENABLE) && (ADI_UART_DMA_EXTENSION_ENABLE == 1)
ADI_UART_RESULT adi_uart_SetTxDmaMode(
                                    ADI_UART_HANDLE const hDevice,
                                    ADI_DMA_MODE    const eMode)
{
    ...
    if(ValidateDmaMode(hDevice, eMode) != ADI_UART_SUCCESS)
    {
        return(ADI_UART_INVALID_PARAMETER);
    }
    ...
    switch (eMode)
    {
        ...
        case ADI_DMA_MODE_PSG:
            hDevice->pChannelTx->pFreeBuffer->nDmaMode = DMA_ENUM_CTL_CYCLE_CTL_PSG_PRI;
            break;
...
/*!
 * @brief        Set the DMA Mode of the UART Rx Channel
 *
* @details       Sets the DMA Mode when ADI_UART_DMA_EXTENSION_ENABLE is enabled.
                 Supported DMA Modes: Basic, Peripheral Scatter-Gather and Ping-Pong.
                 When the ADI_UART_DMA_EXTENSION_ENABLE macro is disabled, which means
 ...
 */
#if defined(ADI_UART_DMA_EXTENSION_ENABLE) && (ADI_UART_DMA_EXTENSION_ENABLE == 1)
ADI_UART_RESULT adi_uart_SetRxDmaMode(
                                    ADI_UART_HANDLE const hDevice,
                                    ADI_DMA_MODE    const eMode)
{
#ifdef ADI_DEBUG
    ...
    switch (eMode)
    {
        ...
        case ADI_DMA_MODE_PSG:
            hDevice->pChannelRx->pFreeBuffer->nDmaMode = DMA_ENUM_CTL_CYCLE_CTL_PSG_PRI;
            break;
...
{
    /* Increment the number of available descriptor */
    pChannel->nFreeDescriptor++;

    if(pChannel->pFillBuffer->nRemDoneTransfers > DMA_TRANSFER_LIMIT)
    {
        pChannel->pFillBuffer->nRemDoneTransfers -= DMA_TRANSFER_LIMIT;
    }
    else
    {
        pChannel->pFillBuffer->nRemDoneTransfers = 0;
    }

    /* If there are remaining bytes to be processed */
    if(pChannel->pFillBuffer->nCount > 0)
    {
        /* Set-up the DMA for the Tx Channel */
        pChannel->pfSubmitBuffer(hDevice, pChannel->pFillBuffer);
    }

    /* If all transactions are acknowledged */
    if(pChannel->pFillBuffer->nRemDoneTransfers ==0)
    {
        /* Manage the buffer */
        uart_ManageProcessedBuffer(hDevice, pChannel, eEvent);
    }
}
#endif
...
void DMA_UART0_TX_Int_Handler(void)
{
    ...
    uart_ManageProcessedBuffer(hDevice,hDevice->pChannelTx,ADI_UART_EVENT_TX_BUFFER_PROCESSED);
#else
    Common_DMA_Uart_Interrupt_Handler(hDevice, hDevice->pChannelTx, ADI_UART_EVENT_TX_BUFFER_PROCESSED);
#endif
#if defined(ADI_CYCLECOUNT_UART_ISR_ENABLED) && (ADI_CYCLECOUNT_UART_ISR_ENABLED == 1u)
...
void DMA_UART0_RX_Int_Handler(void)
{
    ...
    uart_ManageProcessedBuffer(hDevice,hDevice->pChannelRx,ADI_UART_EVENT_RX_BUFFER_PROCESSED);
#else
    Common_DMA_Uart_Interrupt_Handler(hDevice, hDevice->pChannelRx, ADI_UART_EVENT_RX_BUFFER_PROCESSED);
#endif
#if defined(ADI_CYCLECOUNT_UART_ISR_ENABLED) && (ADI_CYCLECOUNT_UART_ISR_ENABLED == 1u)
...
static void uart_RxDataHandler(ADI_UART_HANDLE hDevice, uint8_t nBytes)
{
    ...
    if(activeBufferAvailable)
    {
        ...
        while (activeBufferAvailable)
        {
            ...
            if (!nBytes)
            {
                /* If there are bytes in the current pFillBuffer,
                 * then return the buffer to the application.
                 */
                if (pFillBuffer->nIndex)
                {
...
static void uart_ManageProcessedBuffer(ADI_UART_HANDLE hDevice,ADI_UART_DATA_CHANNEL *pChannel, ADI_UART_EVENT eEvent)
{
     ...
     * if and only if the transfer mode is blocking or not blocking, i.e. if
     * and only if the fill buffer contains a non-0 pStartAddress.
     */
    if (pChannel->eDataTranferMode != ADI_UART_DATA_TRANSFER_MODE_NONE)
    {
    ...
    /**
     * Check if another buffer has already been submitted and is pending.
     * This must be executed early: BEFORE calling the callback function,
     * if there's one, because a new buffer may be submitted by a callback
     * function and this buffer could end up being submitted twice...
     */
    bool startNextTransaction = pChannel->pFillBuffer->bInUse;

    /**
     * The UART Rx interrupt handler and Tx interrupt handler may execute and get interrupted
     * by the other handler. As a result, the IEN register must be accessed with a protection
     * against the other interrupt.
     */
    ADI_INT_STATUS_ALLOC();
    ADI_ENTER_CRITICAL_REGION();
    {
    ...
    /**
     * If there is a callback registered, notify the API that a buffer has been
     * processed. Clean up the buffer. (When a callback function is available,
     * the buffer assigned to  the pActiveBuffer is transmitted back to the
     * application as a callback function parameter, so pActiveBuffer is updated
     * by uart_ManageProcessedBuffer. Otherwise, the application must call
     * adi_uart_GetRxBuffer/adi_uart_GetTxBuffer to get the buffer back, with
     * pActiveBuffer updated in these functions.
     */
    if((hDevice->pfCallback != NULL) && (pChannel->eDataTranferMode == ADI_UART_DATA_TRANSFER_MODE_NONBLOCKING))
    {
...
#if !defined(ADI_UART_DRAIN_RX_BUFFER) || (0==ADI_UART_DRAIN_RX_BUFFER)
        void *pBuffArg = (void*) pBuffer;
#else
        ADI_UART_RX_BUFFER_INFO rxBuffInfo = { (void*)pBuffer, pChannel->pActiveBuffer->nCount, pChannel->pActiveBuffer->nIndex };
        void *pBuffArg = (void*) &rxBuffInfo;
#endif
        ...
        */
        ...
        {
            hDevice->pfCallback(hDevice->pCBParam, (uint32_t)eEvent, pBuffArg);
        }
...
static ADI_UART_RESULT ValidateHandle(ADI_UART_CONST_HANDLE hDevice)
{
 ...
 /*
  * @brief       Validate the input parameters of the DMA mode.
  *
  *
  * @param [in]  hDevice    Device handle obtained from adi_uart_Open().
  * @param [in]  eMode      DMA mode/cycle control
  ...
  *
  * @return      Status
  *    - #ADI_UART_SUCCESS            Specified parameters are valid.
  *    - #ADI_UART_INVALID_PARAMETER  Specified parameters are invalid.
  *
 */
#if defined(ADI_UART_DMA_EXTENSION_ENABLE) && (ADI_UART_DMA_EXTENSION_ENABLE == 1)
static ADI_UART_RESULT ValidateDmaMode(ADI_UART_CONST_HANDLE hDevice,
                                       uint8_t    eMode)
{
    ADI_UART_RESULT nResult;

    switch (eMode)
    {
        case ADI_DMA_MODE_INVALID:
        ...
        default:
            nResult = ADI_UART_INVALID_PARAMETER;
            break;
    }
