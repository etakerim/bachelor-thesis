...
/*!
    ...
    @return     Status
                    ...
                    - #ADI_GPIO_INVALID_PINS        [D] If the Port or Pins parameter is invalid.
...
/*!
    ...
    @return     Status
                    ...
                    - #ADI_GPIO_INVALID_PORT [D] invalid port number
...
ADI_GPIO_RESULT adi_gpio_GetGroupInterruptPins(const ADI_GPIO_PORT Port, const ADI_GPIO_IRQ eIrq, ADI_GPIO_DATA* const pValue)
{
    ...
    /* validate the eIrq */
    if((eIrq != ADI_GPIO_INTA_IRQ) && (eIrq != ADI_GPIO_INTB_IRQ))
    {
        return (ADI_GPIO_INVALID_INTERRUPT);
    }
    
    /* The Port should be valid, as it will be typechecked by the compiler at
     * build time. However, adding a check here for completeness. */
    if((uint16_t)Port >= (uint16_t)(ADI_GPIO_NUM_PORTS))
    {
...
/*!
    @brief      Set/clear the interrupt polarity for the given pins.

    @details    Sets/clears the interrupt polarity for the given pins for the given port.
                When the corresponding bit is set an interrupt is generated when the pin transitions from low-to-high.
                When the corresponding bit is cleared an interrupt is generated when the pin transitions from high-to-low.
    ...
    @sa         adi_gpio_SetGroupInterruptPins
    @sa         adi_gpio_GetGroupInterruptPolarity
    @sa         adi_gpio_GroupInterruptPolarityEnable
*/
...
/*!
    @brief      Determine if the interrupts are generated on the rising or falling edge of
                the corresponding GPIO pin

    @details    Determines if the interrupts are generated on the rising or falling edge of
                the corresponding GPIO pin. Bits set in the pins input are the only ones affected:
    ...
    @param[in]  Port        GPIO port number to be operated on.
    @param[in]  Pins        Pins whose polarity to be set/cleared.
    @param[in]  bLowToHigh  Boolean that defines if interrupts are generated on the rising or falling edge:
...
ADI_GPIO_RESULT  adi_gpio_GroupInterruptPolarityEnable(const ADI_GPIO_PORT Port, const ADI_GPIO_DATA Pins, const bool bLowToHigh)
{
    ...
    if (bLowToHigh) {
      pPort->POL |= Pins;       /* set polarity the bits set in Pins to low to high */
    }else{
      pPort->POL &= ~Pins;      /* set polarity the bits set in Pins to high to low */
    }
...
/*!
    ...
    @param[out] pValue  The passed pValue parameter is written with a packed value containing
                        the status of all the GPIO pins on the given port.
...
/*!
    @brief      Enables/Disables increased Drive Strength capability for GPIO Pin(s)

    @details    The default current sink/source drive strength multiplier is reset to unity
                at power-on reset.  Calling this API with bFlag==True sets the drive strength
                multiplier to twice the default.
...
/*!
    ...
    @sa         adi_gpio_SetLow, adi_gpio_Toggle, adi_gpio_SetData, adi_gpio_GetOutputData
*/
...
/*!
    ...
    @sa         adi_gpio_SetHigh, adi_gpio_Toggle, adi_gpio_SetData, adi_gpio_GetData, adi_gpio_GetOutputData
*/
...
/*!
    ...
    @sa         adi_gpio_SetHigh, adi_gpio_SetLow, adi_gpio_SetData, adi_gpio_GetData, adi_gpio_GetOutputData
*/
...
/*!
    ...
    @sa         adi_gpio_SetHigh, adi_gpio_SetLow, adi_gpio_Toggle, adi_gpio_GetData, adi_gpio_GetOutputData
*/
...
/*!
    ...
    @sa         adi_gpio_SetHigh, adi_gpio_SetLow, adi_gpio_Toggle, adi_gpio_SetData, adi_gpio_GetOutputData
*/
...
/*!
    @brief      Gets the Output Level of all GPIO Pins on a given port
