 ...
 * @details: This a global file which includes a specific file based on the
 *           processor family.
 ...
 * @note This documentation presented here is API documentation only. The device drive user's guide, located in the Documents folder, explains how to use these APIs in an application.  
 * @note The SPI will be configured by default to operate in Master mode. 
 ...
 *                              Tx interrupt occurs when (nMode+1) byte has been transferred.
...
ADI_SPI_RESULT adi_spi_MasterSubmitBuffer (ADI_SPI_HANDLE const hDevice, const ADI_SPI_TRANSCEIVER* const pXfr)
{
    ...
    if( hDevice->bDmaMode==true ) {
      /**
       * There's a race condition between RX and XFRDONE.
       * Let's clear this bit and set it when the SPI DMA interrupt is raised
       * and there are no more bytes to receive/send on the other SPI line. 
       */
...
{ 
    ...
    if( (hDevice->bDmaMode == true) && (hDevice->bRdCtlMode == true) && (pXfr->ReceiverBytes == 1u))
    {
    ...
    if( hDevice->bDmaMode == false) 
    {
      ...
      if( (hDevice->bDmaMode == true) && (pXfr->TransmitterBytes != 0u))
      {
#if defined(ADI_SPI_MASTER_TIM_FOR_DMA) && (ADI_SPI_MASTER_TIM_FOR_DMA != 0u)
        /*set TIM bit to initiate transfer with a write to the SPI_TX register*/
        hDevice->pSpi->CTL |= BITM_SPI_CTL_TIM;
#endif
        /* Spinning waiting for a potential timing window to close is required, but the spin loop needs to be guarded with an exit plan */
        /* After an abitrary number of iterations (say 100) the window should be closed.  */
        for( uint16_t numberOfLoops = 0u; numberOfLoops < 100u; numberOfLoops++)
        {
          /* wait until there is data in the TX FIFO before starting the transaction to avoid a potential race condition resulting in a TX underflow         */
          /* The DMA controller and the SPI controller are on different clock domains, so wait until the DMA starts filling the FIFO before enabling the SPI */
          volatile uint16_t nFifoStatus = hDevice->pSpi->FIFO_STAT;
          /* calculate the number of bytes already written to tx fifo */
          uint16_t bytesInTxFifo = ((BITM_SPI_FIFO_STAT_TX & nFifoStatus) >> BITP_SPI_FIFO_STAT_TX);
          if( bytesInTxFifo != 0u)
          {
            /* There are bytes in Tx FIFO: the SPI transaction can be initiated */
            break;
          }
        }
      }
...
static void common_SPI_Int_Handler (ADI_SPI_DEV_DATA_TYPE* pDD)
{
  ...
  uint16_t nErrorStatusMsk = 0u;

  /* Take a fresh snapshot of FIFO and Clear the RX/TX/IRQ interrupts, as we service those
   * here. */
  pDD->pSpi->STAT = nErrorStatus;
  nFifoStatus = pDD->pSpi->FIFO_STAT;

  /* Trap overflow/underflow errors and terminate the current transaction if there is an error. */
#if !defined(ADI_SPI_TRAP_RXOVR) || (ADI_SPI_TRAP_RXOVR!=0)
  nErrorStatusMsk |= BITM_SPI_STAT_RXOVR;
#endif

#if !defined(ADI_SPI_TRAP_TXUNDR) || (ADI_SPI_TRAP_TXUNDR!=0)
  nErrorStatusMsk |= BITM_SPI_STAT_TXUNDR;
#endif

  if(0u != (nErrorStatusMsk & nErrorStatus)) {
    /* Error condition.  Handle each individually */
    if(BITM_SPI_STAT_TXUNDR == (BITM_SPI_STAT_TXUNDR & nErrorStatus)) {
    ...
    /* There's been an error, so shut it down */
    terminate = true;
  }
  else {
  ...
  if( (pDD->RxRemaining == 0u) && (pDD->TxRemaining == 0u))
  {
    ...
    {
      /* Master mode */
      /* On handler entry we may have had a transfer complete state, so we check
       * if that has happened.  Also, between clearing the int status and reading the 
       * FIFO state, there is a slim chance that the transfer has completed. So,
       * we check both for absolute completeness */
      if((BITM_SPI_STAT_XFRDONE == (nErrorStatus    & BITM_SPI_STAT_XFRDONE)) ||
         (BITM_SPI_STAT_XFRDONE == (pDD->pSpi->STAT & BITM_SPI_STAT_XFRDONE)) )
      {
        /* Master mode XFRDONE */
        terminate = true;

        /* Disable SPI DMA AFTER XFRDONE has been serviced */
        pDD->pSpi->DMA &= (uint16_t)~(BITM_SPI_DMA_EN);
      }
    } else {
  ...
  /* We terminate if:
      - There has been an error on entry to this handler
      - Data has transferred successfully.
     If there has been an error whilst executing this handler (data can transfer
     during this handler, and an error can still occur) then the interrupt will
     have raised, we re-enter the interrupt and then the error is handled above.  */
  if( terminate )  
  {
...
static void common_spi_DMA_Tx(ADI_SPI_HANDLE pDD, const int masterMode)
{
  pDD->TxRemaining = 0u;
  pDD->pSpi->DMA &= (uint16_t)~(BITM_SPI_DMA_TXEN);
  if (0u == pDD->RxRemaining)
  {
    if (masterMode)
    {
      /**
       * Master mode:
       * Now allow the XFRDONE interrupt to occur.
       * It's the SPI ISR that really ends the transaction
...
static void common_spi_DMA_Rx(ADI_SPI_HANDLE pDD, const int masterMode)
{
  pDD->RxRemaining = 0u;
  pDD->pSpi->DMA &= (uint16_t)~(BITM_SPI_DMA_RXEN);
  
  // enable flushing the RxFIFO
  pDD->pSpi->CTL |= (uint16_t)(BITM_SPI_CTL_RFLUSH);
  
  if (0u == pDD->TxRemaining)
  {
    if (masterMode)
    {
      /**
...
{ 
   ISR_PROLOG();
   common_spi_DMA_Tx(spi_device_info[0].hDevice,(ADI_SPI0_MASTER_MODE==1u));
   ISR_EPILOG();       
}
...
{ 
   ISR_PROLOG();
   common_spi_DMA_Rx(spi_device_info[0].hDevice,(ADI_SPI0_MASTER_MODE==1u));
   ISR_EPILOG();
}
...
{ 
   ISR_PROLOG();
   common_spi_DMA_Tx(spi_device_info[1].hDevice,(ADI_SPI1_MASTER_MODE==1u));
   ISR_EPILOG();       
}
...
{ 
   ISR_PROLOG();
   common_spi_DMA_Rx(spi_device_info[1].hDevice,(ADI_SPI1_MASTER_MODE==1u));
   ISR_EPILOG();
}
...
{ 
   ISR_PROLOG();
   common_spi_DMA_Tx(spi_device_info[2].hDevice,(ADI_SPI2_MASTER_MODE==1u));
   ISR_EPILOG();
}
...
{ 
   ISR_PROLOG();
   common_spi_DMA_Rx(spi_device_info[2].hDevice,(ADI_SPI2_MASTER_MODE==1u));
   ISR_EPILOG();
}
