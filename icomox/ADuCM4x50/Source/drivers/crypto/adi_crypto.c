...
/* load AES KEY registers with provided key */
...
/* load 512-bit/32-bytes HMAC KEY registers with provided key */
static void loadHmacKey                      (ADI_CRYPTO_HANDLE const hDevice, uint8_t * const pKey);

#endif  /* ADI_CRYPTO_ENABLE_HMAC_SUPPORT */
#endif  /* defined (__ADUCM4x50__) */
...
static ADI_CRYPTO_RESULT ValidateUserBuffer(ADI_CRYPTO_TRANSACTION * const pBuffer)
{
    ...
    if (
        ...
        ||((pBuffer->numInputBytes  == 0u) && (ADI_CRYPTO_MODE_CCM != pBuffer->eCipherMode))
     ...
     * input data buffer can be larger than MAX_CRYPTO_DMA_BYTES for HMAC and SHA modes only
     */
    if (  (MAX_CRYPTO_DMA_BYTES < pBuffer->numAuthBytes)
       || (MAX_CRYPTO_DMA_BYTES < pBuffer->numOutputBytes)
       || (  (MAX_CRYPTO_DMA_BYTES < pBuffer->numInputBytes)
#if defined (__ADUCM4x50__)
          && (ADI_CRYPTO_MODE_HMAC != pBuffer->eCipherMode)
#endif
          && (ADI_CRYPTO_MODE_SHA != pBuffer->eCipherMode)
          )
       )
    {
        return ADI_CRYPTO_ERR_BAD_BUFFER;
    ...
    {
        /* HMAC output digest is 256-bit and hence the output buffer size should be at least 32 bytes */
        if (pBuffer->numOutputBytes < SHA_OUTPUT_SIZE_IN_BYTES) {
            return ADI_CRYPTO_ERR_BAD_BUFFER;
        }
    }
...
static uint32_t u32FromU8p(uint8_t * const pFourBytes)
{
    return ( (((uint32_t)pFourBytes[3]) << 24ul) | (((uint32_t)pFourBytes[2]) << 16ul) | (((uint32_t)pFourBytes[1]) << 8ul) | (((uint32_t)pFourBytes[0])) );
}


/* load AES KEY register set by length */
static void loadAesKey(uint8_t * const pKey, ADI_CRYPTO_AES_KEY_LEN const keyLen)
{
...
/* load 512-bit/32-bytes HMAC key in KUWn registers */
static void loadHmacKey(ADI_CRYPTO_HANDLE const hDevice, uint8_t * const pKey)
{
    uint32_t volatile *pKeyReg = pREG_CRYPT0_KUW0;
    uint8_t *pUserKey = pKey;
    const uint32_t numKeyWords = 16u;   /* 16 32-bit registers to write: REG_CRYPT0_KUW0 to REG_CRYPT0_KUW15 */

    /**
     * Set KUW key length to 512-bit (all the bits used to define the key size are set)
     * Enable PrKStor before issuing any command
     * Before executing any PrKSTor command, PRKSTOREN must be programmed first and BLKEN next
     */
    SET_BITS(hDevice->pDev->CFG, (uint32_t)ADI_PK_KUW_LEN_512);
...
static void StartCompute(ADI_CRYPTO_HANDLE const hDevice)
{
    ...
    const uint32_t status = hDevice->pDev->STAT;
    ...
    if ((ADI_CRYPTO_MODE_HMAC == pCompute->eCipherMode) && (NULL != pCompute->pHmacKey)) {
        /* load 512-bit HMAC key directly from compute block... */
        loadHmacKey(hDevice, pCompute->pHmacKey);
    }
...
#endif /*ADI_CRYPTO_ENABLE_PKSTOR_SUPPORT */
    {
        ...
        if (NULL != pCompute->pAesKey) {
            loadAesKey(pCompute->pAesKey, pCompute->eAesKeyLen);
        }
    }  /* if PKSTOR / else */
    ...
    /* only enable DMA for non-SHA mode and non-HMAC mode or SHA/HMAC mode with > 4 bytes of input... */
    if (  (true == hDevice->bDmaEnabled)
       && (  (  (ADI_CRYPTO_MODE_SHA != pCompute->eCipherMode)
#if defined (__ADUCM4x50__)
             && (ADI_CRYPTO_MODE_HMAC != pCompute->eCipherMode)
#endif
             )
          || (4u < pCompute->numInputBytesRemaining)
          )
       )
    {
        /* DMA startup... */
...
#if defined (__ADUCM4x50__)
        if(ADI_CRYPTO_MODE_HMAC == pCompute->eCipherMode)
        {
            SET_BITS(hDevice->pDev->STAT, (uint32_t)BITP_CRYPT_STAT_HMACMSGRDY);
        }
#endif /*ADUCM4x50__*/
...
#endif
    {
        ...
        switch (pCompute->eCipherMode) {
                 ...
                 * - INRDYEN must not be used with HMAC or SHA (BITM_CRYPT_INTEN_INRDYEN)
                 * - OUTRDYEN must not be used with HMAC or SHA (BITM_CRYPT_INTEN_OUTRDYEN)
                 */
                SET_BITS(hDevice->pDev->INTEN, (BITM_CRYPT_INTEN_HMACDONEEN | BITM_CRYPT_INTEN_HMACMSGRDYEN | BITM_CRYPT_INTEN_INOVREN));
                break;
#endif /*ADUCM4x50__*/
            case ADI_CRYPTO_MODE_SHA:
...
#if defined (__ADUCM4x50__)
        /**
         * HMAC is fully driven by HMACMSGRDY interrupts: every data fed into the HMAC engine, including the very first ones,
         * is fed in the interruptm handler, when an HMACMSGRDY interrupt is raised.
         * Contrary to SHA, there's no need to call writePioInputData for the first block of data.
         */
        if (ADI_CRYPTO_MODE_HMAC != pCompute->eCipherMode)
#endif
        {
...
static void programDMA(ADI_CRYPTO_HANDLE const hDevice)
{
    ...
    if (0u != pCompute->pNextAuthInput) {
        ...
        /* schedule authentication data into primary descriptor
         * - using ping-pong mode if there's a payload
         * - using basic mode else
         */
        ...
        if (0u == pCompute->numInputBytesRemaining) {
...
#endif  /* #if (ADI_CRYPTO_ENABLE_CBC_SUPPORT == 1) || (ADI_CRYPTO_ENABLE_CCM_SUPPORT == 1) || (ADI_CRYPTO_ENABLE_CTR_SUPPORT == 1) */
    {
        ...
        if (ADI_CRYPTO_MODE_SHA == pCompute->eCipherMode) {
            ...
            num32BitWords = pCompute->numInputBytesRemaining / FIFO_WIDTH_IN_BYTES;
            if (pCompute->numInputBytesRemaining > MAX_CRYPTO_DMA_BYTES) {
                num32BitWords = MAX_CRYPTO_DMA_BYTES / FIFO_WIDTH_IN_BYTES;
                pCCD->DMASRCEND = (uint32_t)pCompute->pNextInput + sizeof(uint32_t) * (num32BitWords - 1u);
            }else{
        ...
        if (ADI_CRYPTO_MODE_HMAC == pCompute->eCipherMode) {
            ...
            pCCD->DMASRCEND = (uint32_t)pCompute->pNextInput + FIFO_WIDTH_IN_BYTES * (num32BitWords - 1u);
        }
        else
#endif
        {
...
static void writePioInputData(ADI_CRYPTO_HANDLE const hDevice, uint32_t const status)
{
...
#endif  /* #if (ADI_CRYPTO_ENABLE_CBC_SUPPORT == 1) || (ADI_CRYPTO_ENABLE_CCM_SUPPORT == 1) || (ADI_CRYPTO_ENABLE_CTR_SUPPORT == 1) */
    {
        ...
        if (ADI_CRYPTO_MODE_SHA == pCompute->eCipherMode) {
            ...
            */
            ...
            if (pCompute->numInputBytesRemaining >= SHA_CHUNK_MAX_BYTES)
            {
                ...
                if(pCompute->numShaBitsRemaining >= SHA_CHUNK_MAX_BITS)
                  pCompute->numShaBitsRemaining  -= SHA_CHUNK_MAX_BITS;
                else
                  pCompute->numShaBitsRemaining = 0;
        ...
        if (ADI_CRYPTO_MODE_HMAC == pCompute->eCipherMode) {
            ...
            /* Drive up to a full "chunk" of HMAC input message data.
               Chunk size is limited to 512-bits (64-bytes) by AES
               hardware compute block.
            */

            if (pCompute->numInputBytesRemaining >= HMAC_CHUNK_MAX_BYTES)
            {
                /* This is the simple case, load up an entire chunk and let it go */
                for (uint8_t i = 0u; i < HMAC_CHUNK_MAX_WORDS; i++) {
                    hDevice->pDev->INBUF = *pCompute->pNextInput;
                    pCompute->pNextInput++;
                }

                if(pCompute->numShaBitsRemaining >= HMAC_CHUNK_MAX_BITS)
                  pCompute->numShaBitsRemaining  -= HMAC_CHUNK_MAX_BITS;
                else
                  pCompute->numShaBitsRemaining = 0;

                pCompute->numInputBytesRemaining -= HMAC_CHUNK_MAX_BYTES;
            }
            else
            {
                /* The final case, we load up any bytes less than a full chunk and trigger the last word */
...
#if  (ADI_CRYPTO_ENABLE_SHA_SUPPORT == 1) || (defined (__ADUCM4x50__) && (ADI_CRYPTO_ENABLE_HMAC_SUPPORT == 1))

#if     (ADI_CRYPTO_ENABLE_SHA_SUPPORT == 1) && ((!defined (__ADUCM4x50__)) || (ADI_CRYPTO_ENABLE_HMAC_SUPPORT == 0))
    /* Copy the SHA output if enabled */
    if (pCompute->eCipherMode == ADI_CRYPTO_MODE_SHA)
#elif   (ADI_CRYPTO_ENABLE_SHA_SUPPORT == 0) && defined (__ADUCM4x50__) && (ADI_CRYPTO_ENABLE_HMAC_SUPPORT == 1)
    /* Copy the HMAC output if enabled */
    if (pCompute->eCipherMode == ADI_CRYPTO_MODE_HMAC)
#else
    /* Copy the SHA or HMAC output if enabled */
    if ((pCompute->eCipherMode == ADI_CRYPTO_MODE_SHA) || (pCompute->eCipherMode == ADI_CRYPTO_MODE_HMAC))
#endif
    {
#if defined (__ADUCM4x50__)
        if (IS_ANY_BIT_SET(status, (BITM_CRYPT_STAT_SHADONE | BITM_CRYPT_STAT_HMACDONE))) {
#else
...
#endif /* (ADI_CRYPTO_ENABLE_SHA_SUPPORT == 1) || (defined (__ADUCM4x50__) && (ADI_CRYPTO_ENABLE_HMAC_SUPPORT == 1)) */
    {
...
void DMA_AES0_IN_Int_Handler (void)
{
    ...
    if (ADI_CRYPTO_MODE_SHA == pCompute->eCipherMode) {
        ...
        if (pCompute->numInputBytesRemaining > MAX_CRYPTO_DMA_BYTES) {
            numTotalBytes            = MAX_CRYPTO_DMA_BYTES;
            num32BitWords            = MAX_CRYPTO_DMA_BYTES / sizeof(uint32_t);
        }
        ...
        if (8u < pCompute->numInputBytesRemaining) {

            /* More DMA transfers required */
            CLR_BITS(hDevice->pDev->CFG, (BITM_CRYPT_CFG_INDMAEN));
            programDMA(hDevice);
            SET_BITS(hDevice->pDev->CFG, (BITM_CRYPT_CFG_INDMAEN));
    ...
    if (ADI_CRYPTO_MODE_HMAC == pCompute->eCipherMode) {
        ...
        /* Update the compute structure to reflect the "post DMA" state of the transaction */
        uint32_t numTotalBytes            = pCompute->numInputBytesRemaining;
        uint32_t num32BitWords            = numTotalBytes / FIFO_WIDTH_IN_BYTES;
        if (pCompute->numInputBytesRemaining > MAX_CRYPTO_DMA_BYTES) {
            numTotalBytes            = MAX_CRYPTO_DMA_BYTES;
            num32BitWords            = numTotalBytes / FIFO_WIDTH_IN_BYTES;
        }
        pCompute->numInputBytesRemaining -= num32BitWords * FIFO_WIDTH_IN_BYTES;
        pCompute->numShaBitsRemaining    -= num32BitWords * FIFO_WIDTH_IN_BITS;
        pCompute->pNextInput             += num32BitWords;

        if (4u < pCompute->numInputBytesRemaining) {
