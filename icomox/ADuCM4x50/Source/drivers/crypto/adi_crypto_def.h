...
/* HMAC hardware max chunk size attributes */
#define HMAC_CHUNK_MAX_BYTES         (64u)
#define HMAC_CHUNK_MAX_BITS          (HMAC_CHUNK_MAX_BYTES * 8U)
#define HMAC_CHUNK_MAX_WORDS         (16u)
...
typedef struct _CRYPTO_COMPUTE {
    ...
    uint8_t                  *pHmacKey;                 /*!< Pointer to 512-bit/32-byte key data pre-formatted as a byte array. */
