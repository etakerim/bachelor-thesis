...
ADI_I2C_RESULT adi_i2c_ReadWrite (ADI_I2C_HANDLE const hDevice, ADI_I2C_TRANSACTION* const pTransaction, uint32_t* const pHwErrors) {
    ...
    *pHwErrors = hDevice->hwErrors;
    ...
    } else if (pTransaction->nDataSize > 0 && pTransaction->bReadNotWrite && (hDevice->pDev->ADDR1 & 0x1) == 0) {
        /* A read was expected but was not performed */
        hDevice->result = ADI_I2C_FAILURE;
    }
...
/*!
 * @brief  Get the number of data written into MTX or read from MRX following the transmission/reception of a transaction.
 *
 * @param[in]     hDevice             Device handle obtained from adi_spi_Open().
 * @param[out]    pDataCount          Pointer to a 32-bit value to store the number of data received/transmitted.
 *
 * @return         Status
 *                - #ADI_I2C_SUCCESS                Call completed successfully.
 *                - #ADI_I2C_BAD_DEVICE_HANDLE [D]  Invalid device handle parameter.
 *
 *  If a transaction, pTransaction, has executed successfully we should have *pDataCount == pTransaction->nDataSize.
 */
ADI_I2C_RESULT adi_i2c_GetNumberOfDataProcessed (ADI_I2C_HANDLE const hDevice, uint32_t *pDataCount) {
    ...
    *pDataCount = hDevice->nBytesProcessed;
...
/*!
 * @brief  Get the errors that have been masked out.
 *
 * @param[in]     hDevice            Device handle obtained from adi_spi_Open().
 * @param[in]     pHwMaskedErrors    Pointer where the masked errors will be written to.
 *
 * @return         Status
 *                - #ADI_I2C_SUCCESS                Call completed successfully.
 *                - #ADI_I2C_BAD_DEVICE_HANDLE [D]  Invalid device handle parameter.
...
ADI_I2C_RESULT adi_i2c_GetHWMaskedErrors (ADI_I2C_HANDLE const hDevice, uint16_t *pHwMaskedErrors) {
    ...
    *pHwMaskedErrors = hDevice->hwMaskedErrors;
...
/*!
 * @brief  Mask out errors. Errors will not be reported.
 *
 * @param[in]     hDevice            Device handle obtained from adi_spi_Open().
 * @param[in]     eMaskedErrors      Error/s to be masked off.
 *
 * @return         Status
 *                - #ADI_I2C_SUCCESS                Call completed successfully.
 *                - #ADI_I2C_BAD_DEVICE_HANDLE [D]  Invalid device handle parameter.
 *                - #ADI_I2C_INVALID_PARAMETER [D]  Invalid input parameter.
 */
ADI_I2C_RESULT adi_i2c_SetHWMaskedErrors (ADI_I2C_HANDLE const hDevice, ADI_I2C_HW_ERRORS eMaskedErrors ) {
    ...
    if (eMaskedErrors > ( ADI_I2C_HW_ERROR_NACK_ADDR        \
                        | ADI_I2C_HW_ERROR_NACK_DATA        \
                        | ADI_I2C_HW_ERROR_ARBITRATION_LOST \
                        | ADI_I2C_HW_ERROR_UNEXPECTED_ERROR \
                        | ADI_I2C_HW_ERROR_MRXOVR           \
                        | ADI_I2C_HW_ERROR_MTXUNDR)){
        return ADI_I2C_INVALID_PARAMETER;
    }
#endif

    hDevice->hwMaskedErrors = eMaskedErrors;
...
/*!
 * @brief  Assign BUSCLR and STOPBUSCLR bit values.
 *
 * @param[in]     hDevice            Device handle obtained from adi_spi_Open().
 * @param[in]     busClr             BUSCLR bit value to assign.
 * @param[in]     stopBusClr         STOPBUSCLR bit value to assign.
 *
 * @return         Status
 *                - #ADI_I2C_SUCCESS                Call completed successfully.
 *                - #ADI_I2C_BAD_DEVICE_HANDLE [D]  Invalid device handle parameter.
...
ADI_I2C_RESULT adi_i2c_SetBusClear (ADI_I2C_HANDLE const hDevice, const bool busClr, const bool stopBusClr) {
    ...
    uint16_t mctl = hDevice->pDev->MCTL;

    if (busClr) {
      mctl |= BITM_I2C_MCTL_BUSCLR;             /* set bit BUSCLR */
    }else{
      mctl &= (~BITM_I2C_MCTL_BUSCLR);          /* clear bit BUSCLR */
    }

    if (stopBusClr) {
      mctl |= BITM_I2C_MCTL_STOPBUSCLR;         /* set bit STOPBUSCLR */
    }else{
      mctl &= (~BITM_I2C_MCTL_STOPBUSCLR);      /* clear bit STOPBUSCLR */
    }

    hDevice->pDev->MCTL = mctl;                 /* update MCTL register */
    return ADI_I2C_SUCCESS;
}
...
static void submitTransaction(ADI_I2C_HANDLE const hDevice, ADI_I2C_TRANSACTION* const pTransaction) {
    ...
    /* reset the number of received bytes. */
    hDevice->nBytesProcessed = 0;
...
static void commenceTransmit(ADI_I2C_HANDLE const hDevice) {
    ...
    while ((hDevice->remainingPrologueCount) && (0u < writableBytes)) {
...
static void transmitHandler(ADI_I2C_HANDLE const hDevice) {
    ...
    /**
     * Clear the IENMTX bit in I2C_MCTL register if there are no more data to
     * transmit. This prevents Tx underflow from being raised, with bit MTXUNDR
     * set in the BITP_I2C_MSTAT register.
     */
    if ((0u == hDevice->remainingPrologueCount) && (0 == hDevice->remainingWriteCount)) {
        const uint16_t bit_IENMTX = (uint16_t) BITM_I2C_MCTL_IENMTX;
        const uint16_t mctl = hDevice->pDev->MCTL & ~bit_IENMTX;
        hDevice->pDev->MCTL = mctl;
    }
...
static void errorHandler(ADI_I2C_HANDLE const hDevice) {
    ...
    if (uZero16 != (hDevice->hwStatus & (uint16_t) BITM_I2C_MSTAT_MRXOVR)) {
        hDevice->hwErrors |= ADI_I2C_HW_ERROR_MRXOVR;
    }

    if (uZero16 != (hDevice->hwStatus & (uint16_t) BITM_I2C_MSTAT_MTXUNDR)) {
        hDevice->hwErrors |= ADI_I2C_HW_ERROR_MTXUNDR;
    }

    /* Mask out errors as configured in adi_SetHWMaskedErrors. */
    hDevice->hwErrors &= ~hDevice->hwMaskedErrors;

    /* if no other errors exist or were masked, note we had an unexpected error */
    if ((hDevice->hwErrors == ADI_I2C_HW_ERROR_NONE) && (uZero16 == hDevice->hwMaskedErrors)) {
