 ...
 *
 * List of DMA registers that are retained in hibernate mode:
 *  - DMA_PDBPTR
 *  - DMA_ADBPTR
 *  - DMA_RMSK_SET
 *  - DMA_RMSK_CLR
...
/**
 * @brief   Force a DMA re-initialization
 *
 * @return         none
 */
void adi_dma_ReInit(void)
{
    pDMA_DevData->Initialized = false;
    adi_dma_Init();
}

/**
 * @brief Enable/Disable the DMA
 * @param [in] enable    true <=> enable DMA
 * 
 * @details Enable/Disable the DMA. This function should not be needed in general as
 *          drivers using the DMA enable it by default.
 */
